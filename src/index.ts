...
const sketch = function (p: p5) {
    const textFont = p.loadFont('fonts/MonaspaceRadon-Regular.otf');
    ...
    function renderShape(shape: Shape) {
        ...
        p.translate(shape.body.position.x - p.width / 2, shape.body.position.y - p.height / 2);
        ...
        switch (shape.type) {
            case 'circle':
                ...
                break;
            case 'text':
                p.text(shape.text!, shape.width! / -2, shape.height! / 3);
                break;
            case 'rect':
